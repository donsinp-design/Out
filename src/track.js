...
(function (OB) {
  ...
  // ---------- population ----------
  const SHOPS = ['shop0', 'shop1', 'shop2', 'shop3', 'shop4', 'shop5', 'shop6', 'shop7', 'shop8', 'shop9', 'shop_seven', 'shop_noodle'];
  const CNSHOPS = ['shopcn0', 'shopcn1', 'shopcn2', 'shopcn3', 'shop2', 'shop5', 'shop8', 'shop_noodle'];
  ...
  function populate(stage, from, to, rng, opts) {
    ...
    for (let i = from; i < to; i++) {
      ...
      // ---- LEFT ----
      if (th.left === 'shop') {
        if (rel % 7 === 0) put(seg, rng.pick(stage.theme === 'chinatown' ? CNSHOPS : SHOPS), edgeL - 0.12, { anchor: 'left' });
        ...
        if (rel % 9 === 3 && rng.chance(0.4)) put(seg, rng.pick(SHOPS), edgeL - 0.12, { anchor: 'left' });
      ...
      if (th.right === 'water') {
        ...
        if (rel % 300 === 250) put(seg, 'chedi', edgeR + rng.range(3, 6), { water: true });
      } else if (th.right === 'shop') {
        if (rel % 7 === 3) put(seg, rng.pick(stage.theme === 'chinatown' ? CNSHOPS : SHOPS), edgeR + 0.12, { anchor: 'right', flip: true });
        ...
        if (rel % 9 === 8 && rng.chance(0.4)) put(seg, rng.pick(SHOPS), edgeR + 0.12, { anchor: 'right', flip: true });
