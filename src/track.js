...
(function (OB) {
  'use strict';
  const T = { segments: [], segLen: 200, roadW: 1800, lanes: 3 };
  OB.track = T;
  ...
  // ---------- population ----------
  const SHOPS = ['shop0', 'shop1', 'shop2', 'shop3', 'shop4', 'shop5', 'shop_seven', 'shop_noodle'];
  const CNSHOPS = ['shopcn0', 'shopcn1', 'shopcn2', 'shopcn3', 'shop2', 'shop5', 'shop_noodle'];
  const FACADES = ['facade0', 'facade1', 'facade1', 'facade2', 'facade3', 'facade4', 'facade5'];
  const TOWERS = ['tower0', 'tower1', 'tower2', 'tower3', 'tower4', 'tower5', 'tower6', 'tower7'];
  // shophouse picker: mostly the facades cut from the reference, some procedural ones for variety
  function pickShop(rng, theme) {
    if (theme === 'chinatown') return rng.chance(0.5) ? rng.pick(['facade2', 'facade5', 'facade4', 'facade1']) : rng.pick(CNSHOPS);
    return rng.chance(0.72) ? rng.pick(FACADES) : rng.pick(SHOPS);
  }
  ...
  function populate(stage, from, to, rng, opts) {
    ...
    for (let i = from; i < to; i++) {
      ...
      // ---- LEFT ----
      if (th.left === 'shop') {
        if (rel % (stage.theme === 'chinatown' ? 10 : 12) === 0) put(seg, pickShop(rng, stage.theme), edgeL - 0.12, { anchor: 'left' });
        ...
        if (rel % 9 === 3 && rng.chance(0.4)) put(seg, rng.pick(['facade1', 'facade4', 'shop2', 'shop5']), edgeL - 0.12, { anchor: 'left' });
      ...
      if (th.right === 'water') {
        seg.rail = true;
        if (rel % 24 === 12) put(seg, 'lamp', edgeR - 0.06);
        if (rel % 230 === 150) put(seg, 'spirit', edgeR - 0.12);
        if (rel % 330 === 60) put(seg, 'thatien_pole', edgeR - 0.08);
        if (rel % 60 === 30) put(seg, rng.pick(['boat0', 'boat1', 'boat2']), edgeR + rng.range(1.2, 3.5), { water: true });
      } else if (th.right === 'shop') {
        if (rel % (stage.theme === 'chinatown' ? 10 : 12) === 5) put(seg, pickShop(rng, stage.theme), edgeR + 0.12, { anchor: 'right' });
        ...
        if (rel % 9 === 8 && rng.chance(0.4)) put(seg, rng.pick(['facade1', 'facade3', 'shop0', 'shop4']), edgeR + 0.12, { anchor: 'right' });
  ...
  T.buildStage = function (stageKey, stageNo, seed) {
    ...
    const rng = OB.rng(seed + stageNo * 7919);
