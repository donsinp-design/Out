...
(function (OB) {
  ...
  T.THEMES = {
    riverside: { left: 'shop', right: 'water', light: 'day', curv: 1.6, hills: 0.3, straight: 0.12, traffic: { density: 9, min: 0.28, max: 0.62, oncoming: 0 } },
    chinatown: { left: 'shop', right: 'shop', light: 'day', curv: 1.1, hills: 0.2, traffic: { density: 13, min: 0.22, max: 0.55, oncoming: 0 } },
  ...
  function populate(stage, from, to, rng, opts) {
    ...
    for (let i = from; i < to; i++) {
      ...
      if (th.left === 'shop') {
        ...
        if (rel % 62 === 27) put(seg, 'tuktuk_parked' + rng.int(5), edgeL + 0.11, { flip: true });   // waiting at the kerb
        ...
        if (rel % 70 === 46) put(seg, 'tuktuk_parked' + rng.int(5), edgeL + 0.11, { flip: true });
        ...
        if (rel % 66 === 31) put(seg, 'tuktuk_parked' + rng.int(5), edgeL + 0.11, { flip: true });
      }
      // ---- RIGHT ----
      if (th.right === 'water') {
        seg.rail = true;
        // everything on the river side stands on the walkway (1.0 .. 1.13), never past the railing
        if (rel % 24 === 12) put(seg, 'lamp', edgeR - 0.21);   // arm reaches back over the walkway, not the river
        if (rel % 230 === 150) put(seg, 'spirit_small', edgeR - 0.13);
        if (rel % 330 === 60) put(seg, 'thatien_pole', edgeR - 0.12);
        if (rel % 60 === 30) put(seg, 'boat', edgeR + rng.range(1.2, 3.5), { water: true, flip: rng.chance(0.5) });
        ...
        if (rel % 70 === 12) put(seg, 'tuktuk_parked' + rng.int(5), edgeR - 0.11);
        ...
        if (rel % 66 === 52) put(seg, 'tuktuk_parked' + rng.int(5), edgeR - 0.11);
