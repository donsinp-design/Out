...
(function (OB) {
  ...
  function update(dt) {
    ...
    if (mode === 'play') {
      ...
      G.cleanT += dt;
      if (G.cleanT >= 10) { G.cleanT -= 10; G.health = Math.min(100, G.health + 6); G.ice = Math.min(100, G.ice + 8); G.score += 1000; say('CLEAN STREAK', 'ไม่มีอุบัติเหตุ 10 วินาที  ·  +1000', 1.8, '#7CFF7C', 14); A.sfx('check'); }
      progress(seg);
    } else if (mode === 'goal') {
      G.goalT += dt;
