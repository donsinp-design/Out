...
(function (OB) {
  ...
  function checkCollisions(seg) {
    ...
    for (const c of G.cars) {
      ...
      // Near-dodge tracking: a single-frame "is the gap in this thin band right now" check can't reliably see a
      // close pass, because the lateral speed during a real weave (or a fast car itself) covers more ground per
      // frame than the band is wide, jumping straight from "clearly clear" to "already touching" between two
      // frames. So instead the closest clearance reached over the whole encounter is tracked every frame the
      // vehicle is nearby (clearance measured from the same crash boundary the collision check below uses), and
      // the verdict — a clean near miss worth a bonus, or a touch that doesn't count — is settled once the
      // vehicle has fully dropped behind. Bigger vehicles keep their old wider flinch; oncoming traffic pays more.
      if (!c.whooshed && Math.abs(dz) < segLen * 1.2) {
        ...
        if (clearance < 0) c.everHit = true;
        else if (c.minClear === undefined || clearance < c.minClear) c.minClear = clearance;
      }
      if (!c.whooshed && dz < -segLen * 0.6 && c.minClear !== undefined) {
        c.whooshed = true;
        if (!c.everHit && c.minClear < (PLAYER_W + c.spr.w / RW) * 0.25 && Math.abs(G.speed - c.speed) > G.maxSpeed * 0.2) {
